...
- `packages/realtime` — channel name helpers, zod-validated event schemas, React subscribe hook, client/server publish APIs. **Only legal importer of `@supabase/supabase-js` realtime APIs in the workspace.** v0 client publishes proxy through `api.cropautonomy.com/v1/realtime/publish`; subscribes use the anon client with channel-name tenant scoping. See [`docs/architecture/realtime-package-spec.md`](docs/architecture/realtime-package-spec.md).
- `packages/analytics` — typed PostHog wrapper. **Only legal importer of `posthog-js` in the workspace.** `@gaia/analytics` exposes `capture()`/`identify()`/etc (safe no-op until `initAnalytics` runs with a key), `@gaia/analytics/events` the typed event registry, `@gaia/analytics/next` the App Router `AnalyticsProvider`. Consumed by Next apps via `transpilePackages`. See [`docs/architecture/queueing-email-analytics.md`](docs/architecture/queueing-email-analytics.md) § Analytics Implementation.
...
API surface in `services/api` at `api.cropautonomy.com` (cross-origin from field.cropautonomy.com, Clerk session cookie scoped to `.cropautonomy.com`):

- `POST /v1/captures` — reserves a `captures` row, mints a signed PUT URL into `scan-originals/org/{orgId}/capture/{captureId}.{ext}` (server-chosen path), responds with `{ captureId, uploadUrl, uploadToken, expiresAt }`. Validates the caller is `technician`-or-higher in the referenced org and that any farm/field/zone/cropType/session IDs belong to that org. ([route](services/api/src/routes/captures.ts))
- `POST /v1/captures/{id}/finalize` — verifies the object exists at the expected size, transitions `pending_upload → uploaded → analysis_queued`, inserts an `analysis_jobs` row. Optional `thumbnailDataUrl` is uploaded to `{path}_thumb.jpg`. ([route](services/api/src/routes/captures.ts))
- `POST /v1/captures/{id}/reanalyze` — re-queues analysis for a `failed` capture (Storage object already exists): inserts a fresh `analysis_jobs` row, points the capture back at it (`status → analysis_queued`, clears `status_message`), and re-enqueues `scan.analysis.requested`. Mirrors the re-queue tail of `finalize`; 409s on any non-`failed` status. Drives the portal's per-capture Retry control. ([route](services/api/src/routes/captures.ts))
- `POST /v1/capture-sessions` — starts a session row + publishes `capture.session.started`. ([route](services/api/src/routes/capture-sessions.ts))
- `PATCH /v1/capture-sessions/{id}` — `{ action: "pause" | "resume" | "end" }` + publishes the matching lifecycle event. ([route](services/api/src/routes/capture-sessions.ts))
- `GET /v1/fields` — returns the operator's org-scoped fields with PostGIS geometries serialized as GeoJSON (`boundary` polygons + `centroid` points). Backed by the `list_org_fields` RPC in [`0007_fields_geojson_rpc.sql`](packages/db/migrations/0007_fields_geojson_rpc.sql). Consumed by the field PWA's `/map` view. ([route](services/api/src/routes/fields.ts))
- `POST /v1/realtime/publish` — proxy publish from the browser. Validates the event against the `@gaia/realtime` zod registry and re-broadcasts via service role. Used by the field PWA until the Clerk → Supabase JWT bridge is in place. ([route](services/api/src/routes/realtime.ts))
- Clerk webhook (mirrors identity into `public.users`) lives in [portal-web](apps/portal-web/app/api/webhooks/clerk/route.ts) — `CLERK_WEBHOOK_SECRET` required.
...
When extending the capture pipeline, update **four** places together: the SQL in [`packages/db/migrations/0004_captures_and_analysis.sql`](packages/db/migrations/0004_captures_and_analysis.sql), the request schema in [`services/api/src/routes/captures.ts`](services/api/src/routes/captures.ts), the IndexedDB record + drain worker in [`apps/field-web/src/lib/db.ts`](apps/field-web/src/lib/db.ts) + [`upload.ts`](apps/field-web/src/lib/upload.ts), and the event schemas in [`packages/realtime/src/events.ts`](packages/realtime/src/events.ts) if a new lifecycle event is added.
...
v0 transports: subscribes go direct to Supabase Realtime broadcast with the anon key (channel-name tenancy); browser publishes proxy through `api.cropautonomy.com/v1/realtime/publish` (server holds the service role). Swapping to direct browser publish is a one-line `configurePublishFromClient({ kind: "supabase", … })` change in `apps/field-web/src/main.tsx` once the Clerk → Supabase JWT template is in place.
