...
const farmsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get<{ Querystring: { teamId?: string; mine?: string } }>(
    ...
    async (request, _reply) => {
      ...
      // Team assignments per farm + whether the caller may edit them (drives the
      // farm modal's team selector; teams.assign, manager+).
      const teamsByFarm = await teamIdsByResource(
        supabase,
        caller.orgId,
        "farm",
        rows.map((r) => r.id)
