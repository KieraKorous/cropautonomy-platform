...
import { DiscardedRecordings } from "./DiscardedRecordings";
...
export default async function SettingsPage() {
  ...
  let discardedRecordings: CaptureSummary[] = [];
  let loadError: string | null = null;
  let recordingsLoadError: string | null = null;
  ...
  try {
    const result = await listDiscardedRecordings();
    discardedRecordings = result.captures;
  } catch (err) {
    recordingsLoadError =
      err instanceof ApiError ? err.message : "Could not reach the captures service.";
  }
  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <section className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <h2 className="text-base font-semibold text-neutral">Discarded recordings</h2>
          <p className="max-w-2xl text-sm text-base-content/65">
            Recordings you discard are hidden from the recordings list but kept in storage.
            Permanently delete them here to free up space — this removes the video and cannot be
            undone.
          </p>
        </div>
        ...
        {recordingsLoadError ? (
          <section className="flex flex-col items-start gap-3 rounded-xl border border-dashed border-base-content/20 bg-base-100 px-6 py-8">
            ...
            <p className="max-w-xl text-sm text-base-content/65">
              Discarded recordings aren&apos;t loading right now. Refresh in a moment — if it keeps
              happening, make sure you have an active organization or try again shortly.
            </p>
            <p className="text-xs text-base-content/40">{recordingsLoadError}</p>
          </section>
        ) : (
          <DiscardedRecordings recordings={discardedRecordings} />
        )}
      </section>
    </div>
  );
