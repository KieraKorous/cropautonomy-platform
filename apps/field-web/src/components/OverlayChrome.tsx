...
export function OverlayChrome({
  ...
  return (
    <>
      ...
      <div className="safe-top pointer-events-none fixed left-0 top-0 z-30 flex items-center gap-1.5 px-3 py-2">
      ...
      {sessionStatus !== "off" && (
        <div className="safe-top pointer-events-none fixed left-1/2 top-0 z-30 -translate-x-1/2 px-3 py-2">
          <SessionPill status={sessionStatus} variant={variant} />
        </div>
      )}

      {/* Top-right account */}
      <div className="safe-top pointer-events-none fixed right-0 top-0 z-30 px-3 py-2">
...
function Dot({
  ...
  const className = expanded
    ? `pointer-events-auto flex h-8 items-center gap-1.5 rounded-full px-2.5 text-xs font-medium ${surface}`
    : `pointer-events-auto flex h-8 w-8 items-center justify-center rounded-full ${surface}`;

  const dot = <span className={`block h-2 w-2 rounded-full ${toneColor}`} />;
