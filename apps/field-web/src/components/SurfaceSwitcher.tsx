...
export function SurfaceSwitcher({ variant = "dark" }: { variant?: OverlayVariant }) {
    ...
    return (
        <div className="safe-bottom pointer-events-none fixed bottom-0 left-1/2 z-30 -translate-x-1/2 px-4 pb-6 mb-4">
            <nav
                className={`pointer-events-auto flex h-14 items-stretch overflow-hidden rounded-full p-1 ${surface}`}
...
function SwitchButton({
    ...
    return (
        <Link
            ...
            aria-label={label}
            aria-pressed={active}
            className={`flex items-center gap-2 rounded-full px-5 text-sm font-semibold transition ${active ? activeStyle : inactiveStyle
                }`}
        >
            {children}
            <span>{label}</span>
